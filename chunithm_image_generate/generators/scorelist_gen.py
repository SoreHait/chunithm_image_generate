...
class ChuniScoreListGenerate:
    ...
    def generate_scorelist_layout(score_list: scorelist.ScoreList, api: str) -> Image.Image:
        ...
        if len(score_list.score_list) == 0:
            raise ValueError(f"User ScoreList has no records. Using API: {api}")
        ...
        component = Image.open(os.path.join(RESOURCE_BACKGROUND_PATH, 'bg_overlay.png'))
        ...
        for constant_group in constant_categorized:
            ...
            for idx, record in enumerate(constant_group):
                ...
                song = ChuniScoreListGenerate.generate_single_song(record).resize(scale_to)
